...
import {inventoryAdapterDTO, inventoryAdapterEntity} from '../adapter/inventory.adapter.js';
...
export class InventoryService {
    ...
    static async create(inventory) {
        try {
            const inventoryExiste = await InventoryRepository.findByExist(inventory.ip, inventory.mac, inventory.reference);
            if (!inventoryExiste ) {
                console.log("Esto es lo que llega")
                console.log(inventory)
                return await InventoryRepository.create(inventoryAdapterEntity(inventory));
            }
            throw new Error("La dirección IP, MAC o referencia ya se encuentra registrada");
        } catch (error) {
