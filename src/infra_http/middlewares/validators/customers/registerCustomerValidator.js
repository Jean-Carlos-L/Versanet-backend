...
class UniqueEmailHandler extends Handler {
  async handle(body) {
    ...
    if (customer) return { ok: false, error: "email already registered" };
    return super.handle(body);
  }
}
...
class UniqueDocumentHandler extends Handler {
  async handle(body) {
    ...
    if (customer) return { ok: false, error: "document already registered" };
    return super.handle(body);
  }
}
...
function buildRegisterValidatorChain() {
  ...
  const e = new UniqueDocumentHandler();
